...
IPSFModel::IPSFModel(int img_size_i, int img_size_j, int img_size_k) :
...
bool IPSFModel::initialize(const char* bt_filename, const char* ba_filename)
{
	SystemLog::write("initializing ipsf model ...\n");
...
void IPSFModel::blur(Image<float>& img_blur, const Image<float>& img, BLUR_type type) const
{
    int ni = img.getDimI();
    int nj = img.getDimJ();
    int nk = img.getDimK();
...
void IPSFModel::blurAxialK(Image<float>& img_blur,
